...
float g_videoZoomMultiplier = 1.0f; // relative to auto-fit; 1.0 means fit-to-window
float g_videoPanNormX = 0.5f;
float g_videoPanNormY = 0.5f;
bool  g_videoPanning = false;
POINT g_videoPanDragStart{};
float g_videoPanStartNormX = 0.5f;
float g_videoPanStartNormY = 0.5f;
...
enum class FileOpKind { ClipboardPaste, DeleteFiles, CopyToPath, TopazSubmit, Iw3Submit };
...
// ----------------------------- IW3 2D->3D submit options

struct Iw3JobOptions {
    // Stereo params
    double       divergence = 1.8;              // 0-2 "reasonable" per iw3, >2 for fun
    double       convergence = 0.5;              // 0-1 screen plane
    std::wstring method = L"row_flow_v3";   // row_flow / row_flow_v3 / row_flow_v3_sym
    std::wstring depthModel = L"Any_V2_N_B";    // good default
    std::wstring syntheticView = L"right";       // keep left original, synth right

    int preserveScreenBorder = 1;                // 1/0 -> emits --preserve-screen-border
    int emaNormalize = 1;                // emits --ema-normalize
    int sceneDetect = 1;                // emits --scene-detect
    int tta = 0;                // emits --tta
    int depthAA = 0;                // emits --depth-aa

    // Video/encode
    int          gpu = 0;                // --gpu 0
    int          maxFps = 30;               // --max-fps 30
    std::wstring videoFormat = L"mp4";           // --video-format mp4
    std::wstring videoCodec = L"hevc_nvenc";    // --video-codec hevc_nvenc (fallback to h264_nvenc if needed)
    int          crf = 20;               // --crf 20
    std::wstring preset = L"p5";            // --preset p5
    std::wstring pixFmt = L"yuv420p";       // --pix-fmt yuv420p

    // Output mode: default FULL SBS (do NOT pass --half-sbs)
    int          halfSbs = 0;                // if 1 -> pass --half-sbs
};
...
static bool PromptIw3PresetModal(Iw3JobOptions& outOpt)
{
    ...
    if (!reg) {
        ...
        wc.lpszClassName = L"Iw3PresetClass";
        ...
        wc.lpfnWndProc = [](HWND hh, UINT mm, WPARAM ww, LPARAM ll) -> LRESULT {
            ...
            case WM_CREATE: {
                ...
                int btnW = DpiScale(520), btnH = DpiScale(28);
                int y = margin;

                HWND lbl = CreateWindowExW(0, L"STATIC",
                    L"IW3 2D ? 3D preset (Full SBS):",
                ...
                addBtn(9301, L"1) Fast preview (less stable depth)  [div=1.2  method=row_flow  model=ZoeD_Any_N]");
                addBtn(9302, L"2) Recommended (stable)             [div=1.8  method=row_flow_v3  model=Any_V2_N_B]");
                addBtn(9303, L"3) Strong (more pop)                [div=2.2  method=row_flow_v3  model=Any_V2_N_B]");
                addBtn(9304, L"4) Max quality (slow)               [div=1.8  method=row_flow_v3_sym  model=Any_V3_Mono + TTA + AA]");
                addBtn(9305, L"5) Max quality + strong (slow)      [div=2.2  method=row_flow_v3_sym  model=Any_V3_Mono + TTA + AA]");
                addBtn(9306, L"6) Wild (artifacts possible)         [div=2.6  method=row_flow_v3_sym  model=Any_V3_Mono + TTA + AA]");
                return 0;
            }
            case WM_COMMAND: {
                int id = LOWORD(ww);
    ...
    int W = DpiScale(580), H = DpiScale(290);
    int X = wa.left + ((wa.right - wa.left) - W) / 2;
    int Y = wa.top + ((wa.bottom - wa.top) - H) / 2;

    HWND wnd = CreateWindowExW(WS_EX_DLGMODALFRAME | WS_EX_TOPMOST, L"Iw3PresetClass",
        L"IW3 2D?3D", WS_POPUPWINDOW | WS_CAPTION | WS_SYSMENU | WS_VISIBLE,
    ...
    outOpt = Iw3JobOptions{};
    outOpt.divergence = 1.8;
    outOpt.convergence = 0.5;
    outOpt.method = L"row_flow_v3";
    outOpt.depthModel = L"Any_V2_N_B";
    outOpt.syntheticView = L"right";
    outOpt.preserveScreenBorder = 1;
    outOpt.emaNormalize = 1;
    outOpt.sceneDetect = 1;
    outOpt.tta = 0;
    outOpt.depthAA = 0;
    outOpt.halfSbs = 0; // FULL SBS
    outOpt.gpu = 0;
    outOpt.maxFps = 30;
    outOpt.videoFormat = L"mp4";
    outOpt.videoCodec = L"hevc_nvenc";
    outOpt.crf = 20;
    outOpt.preset = L"p5";
    outOpt.pixFmt = L"yuv420p";

    auto setFast = [&]() {
        outOpt.method = L"row_flow";
        outOpt.depthModel = L"ZoeD_Any_N";
        outOpt.emaNormalize = 0;
        outOpt.sceneDetect = 0;
        outOpt.tta = 0;
        outOpt.depthAA = 0;
        };
    auto setMaxQ = [&]() {
        outOpt.method = L"row_flow_v3_sym";
        outOpt.depthModel = L"Any_V3_Mono";
        outOpt.emaNormalize = 1;
        outOpt.sceneDetect = 1;
        outOpt.tta = 1;
        outOpt.depthAA = 1;
    ...
    switch (ctx.id) {
    case 9301: outOpt.divergence = 1.2; setFast(); break;
    case 9302: outOpt.divergence = 1.8; break;
    case 9303: outOpt.divergence = 2.2; break;
    case 9304: outOpt.divergence = 1.8; setMaxQ(); break;
    case 9305: outOpt.divergence = 2.2; setMaxQ(); break;
    case 9306: outOpt.divergence = 2.6; setMaxQ(); break;
    default: break;
    }
    return true;
}
...
static void BuildIw3JobJsonUtf8(const std::wstring& originalFull,
    const std::wstring& queuedFileNameOnly,
    const Iw3JobOptions& opt,
    std::string& outUtf8)
{
    ...
    auto ws2u8 = [&](const std::wstring& w) -> std::string {
        return JsonEscapeUtf8(ToUtf8(w));
        };

    char divBuf[64]; sprintf_s(divBuf, "%.4f", opt.divergence);
    char convBuf[64]; sprintf_s(convBuf, "%.4f", opt.convergence);
    ...
    oss
        ...
        << "  \"divergence\": " << divBuf << ",\n"
        << "  \"convergence\": " << convBuf << ",\n"
        << "  \"method\": \"" << ws2u8(opt.method) << "\",\n"
        << "  \"depth_model\": \"" << ws2u8(opt.depthModel) << "\",\n"
        << "  \"synthetic_view\": \"" << ws2u8(opt.syntheticView) << "\",\n"
        << "  \"preserve_screen_border\": " << (opt.preserveScreenBorder ? 1 : 0) << ",\n"
        << "  \"ema_normalize\": " << (opt.emaNormalize ? 1 : 0) << ",\n"
        << "  \"scene_detect\": " << (opt.sceneDetect ? 1 : 0) << ",\n"
        << "  \"tta\": " << (opt.tta ? 1 : 0) << ",\n"
        << "  \"depth_aa\": " << (opt.depthAA ? 1 : 0) << ",\n"
        << "  \"half_sbs\": " << (opt.halfSbs ? 1 : 0) << ",\n"
        << "  \"gpu\": " << opt.gpu << ",\n"
        << "  \"max_fps\": " << opt.maxFps << ",\n"
        << "  \"video_format\": \"" << ws2u8(opt.videoFormat) << "\",\n"
        << "  \"video_codec\": \"" << ws2u8(opt.videoCodec) << "\",\n"
        << "  \"crf\": " << opt.crf << ",\n"
        << "  \"preset\": \"" << ws2u8(opt.preset) << "\",\n"
        << "  \"pix_fmt\": \"" << ws2u8(opt.pixFmt) << "\"\n"
        << "}\n";
...
static void SetTitlePlaying() {
    ...
    if (g_videoZoomMultiplier > 1.001f) {
        wchar_t zoomBuf[32];
        int zoomPct = (int)(g_videoZoomMultiplier * 100.0f + 0.5f);
...
static void ShowHelp() {
    ...
    msg += L"  Ctrl+3               : Submit selected videos to IW3 2D?3D queue (writes .json jobs)\n";
...
static POINT PointFromLParam(LPARAM l) {
    POINT pt{};
...
static int VideoAxisTravel(int renderSize, int clientSize) {
    if (renderSize > clientSize) return renderSize - clientSize;
    if (clientSize > renderSize) return clientSize - renderSize;
    return 0;
}

static int VideoAxisPosFromNorm(float norm, int renderSize, int clientSize) {
    int travel = VideoAxisTravel(renderSize, clientSize);
    if (travel <= 0) return 0;
    if (renderSize >= clientSize) return -(int)(Clamp01(norm) * (float)travel + 0.5f);
    return (int)(Clamp01(norm) * (float)travel + 0.5f);
}

static float VideoAxisNormFromPos(double pos, int renderSize, int clientSize) {
    int travel = VideoAxisTravel(renderSize, clientSize);
    if (travel <= 0) return 0.5f;
    if (renderSize >= clientSize) return Clamp01((float)((-pos) / (double)travel));
    return Clamp01((float)(pos / (double)travel));
}

struct VideoViewMetrics {
    unsigned videoW = 0;
    unsigned videoH = 0;
    int clientW = 0;
    int clientH = 0;
...
static bool RefreshVideoSourceSize() {
    if (!g_mp) return false;

    unsigned videoW = 0, videoH = 0;
    if (libvlc_video_get_size(g_mp, 0, &videoW, &videoH) != 0 || videoW == 0 || videoH == 0) {
        return false;
    }

    g_videoSourceW = videoW;
    g_videoSourceH = videoH;
    return true;
}

static bool GetVideoViewMetrics(VideoViewMetrics& out, float zoomOverride = -1.0f) {
    if (!g_hwndVideo || !IsWindow(g_hwndVideo)) return false;

    if ((g_videoSourceW == 0 || g_videoSourceH == 0) && !RefreshVideoSourceSize()) {
        return false;
    }

    out.videoW = g_videoSourceW;
    out.videoH = g_videoSourceH;

    RECT rc{};
    GetClientRect(g_hwndVideo, &rc);
    out.clientW = rc.right - rc.left;
    out.clientH = rc.bottom - rc.top;
    if (out.clientW <= 0 || out.clientH <= 0) return false;

    double fitScaleX = (double)out.clientW / (double)out.videoW;
    double fitScaleY = (double)out.clientH / (double)out.videoH;
    double fitScale = (fitScaleX < fitScaleY) ? fitScaleX : fitScaleY;
    if (fitScale <= 0.0) return false;

    out.fitW = (int)((double)out.videoW * fitScale + 0.5);
    out.fitH = (int)((double)out.videoH * fitScale + 0.5);
    if (out.fitW < 1) out.fitW = 1;
    if (out.fitH < 1) out.fitH = 1;

    float zoom = (zoomOverride > 0.f) ? zoomOverride : g_videoZoomMultiplier;
    if (zoom < 1.0f) zoom = 1.0f;

    out.renderW = (int)((double)out.fitW * (double)zoom + 0.5);
    out.renderH = (int)((double)out.fitH * (double)zoom + 0.5);
    if (out.renderW < 1) out.renderW = 1;
    if (out.renderH < 1) out.renderH = 1;

    out.maxPanX = VideoAxisTravel(out.renderW, out.clientW);
    out.maxPanY = VideoAxisTravel(out.renderH, out.clientH);

    out.renderX = VideoAxisPosFromNorm(g_videoPanNormX, out.renderW, out.clientW);
    out.renderY = VideoAxisPosFromNorm(g_videoPanNormY, out.renderH, out.clientH);
    return true;
}
static void StopVideoPanDrag() {
    g_videoPanning = false;
    if (GetCapture() == g_hwndVideo) ReleaseCapture();
}

static void SyncEmbeddedVideoChildren(int surfaceX, int surfaceY, int surfaceW, int surfaceH) {
    if (surfaceW < 1) surfaceW = 1;
    if (surfaceH < 1) surfaceH = 1;

    if (g_hwndVideo && IsWindow(g_hwndVideo)) {
        for (HWND child = GetWindow(g_hwndVideo, GW_CHILD); child; child = GetWindow(child, GW_HWNDNEXT)) {
            if (child == g_hwndVideoSurface) continue;
            SetWindowPos(child, NULL, surfaceX, surfaceY, surfaceW, surfaceH,
                SWP_NOZORDER | SWP_NOACTIVATE | SWP_SHOWWINDOW);
        }
    }

    if (g_hwndVideoSurface && IsWindow(g_hwndVideoSurface)) {
        for (HWND child = GetWindow(g_hwndVideoSurface, GW_CHILD); child; child = GetWindow(child, GW_HWNDNEXT)) {
            SetWindowPos(child, NULL, 0, 0, surfaceW, surfaceH,
                SWP_NOZORDER | SWP_NOACTIVATE | SWP_SHOWWINDOW);
        }
    }
}
static void ApplyVideoZoom() {
    if (!g_hwndVideo || !g_hwndVideoSurface || !IsWindow(g_hwndVideo) || !IsWindow(g_hwndVideoSurface)) return;

    if (g_mp) {
        libvlc_video_set_scale(g_mp, 0.f);
        libvlc_video_set_aspect_ratio(g_mp, NULL);
        libvlc_video_set_crop_geometry(g_mp, NULL);
    }

    VideoViewMetrics m{};
    if (!GetVideoViewMetrics(m)) {
        RECT rc{};
        GetClientRect(g_hwndVideo, &rc);
        ...
        if (w < 1) w = 1;
        if (h < 1) h = 1;
        MoveWindow(g_hwndVideoSurface, 0, 0, w, h, TRUE);
        SyncEmbeddedVideoChildren(0, 0, w, h);
        RedrawWindow(g_hwndVideoSurface, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ALLCHILDREN);
        RedrawWindow(g_hwndVideo, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ALLCHILDREN);
        return;
    }

    MoveWindow(g_hwndVideoSurface, m.renderX, m.renderY, m.renderW, m.renderH, TRUE);
    SyncEmbeddedVideoChildren(m.renderX, m.renderY, m.renderW, m.renderH);
    RedrawWindow(g_hwndVideoSurface, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ALLCHILDREN);
    RedrawWindow(g_hwndVideo, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ALLCHILDREN);
}

static void ResetVideoZoom() {
    g_videoZoomMultiplier = 1.0f;
    g_videoPanNormX = 0.5f;
    g_videoPanNormY = 0.5f;
    g_videoSourceW = 0;
    g_videoSourceH = 0;
    StopVideoPanDrag();
    if (g_mp) {
...
static void AdjustVideoZoom(bool zoomIn, const POINT* anchorClient = nullptr) {
    const float kZoomStep = 0.25f;
    const float kZoomMin = 1.0f;
    const float kZoomMax = 4.0f;

    float oldZoom = g_videoZoomMultiplier;
    float newZoom = oldZoom + (zoomIn ? kZoomStep : -kZoomStep);
    if (newZoom > kZoomMax) newZoom = kZoomMax;
    if (newZoom < (kZoomMin + 0.001f)) newZoom = kZoomMin;

    if (oldZoom <= 1.001f) RefreshVideoSourceSize();

    VideoViewMetrics oldM{};
    bool haveOld = GetVideoViewMetrics(oldM, oldZoom);

    POINT anchor{};
    double sourceFracX = 0.5;
    double sourceFracY = 0.5;
    if (haveOld) {
        anchor = anchorClient ? *anchorClient : POINT{ oldM.clientW / 2, oldM.clientH / 2 };

        RECT imageRc{ oldM.renderX, oldM.renderY, oldM.renderX + oldM.renderW, oldM.renderY + oldM.renderH };
        if (anchor.x < imageRc.left) anchor.x = imageRc.left;
        if (anchor.y < imageRc.top) anchor.y = imageRc.top;
        if (anchor.x > imageRc.right) anchor.x = imageRc.right;
        if (anchor.y > imageRc.bottom) anchor.y = imageRc.bottom;

        if (oldM.renderW > 0) sourceFracX = ((double)anchor.x - (double)oldM.renderX) / (double)oldM.renderW;
        if (oldM.renderH > 0) sourceFracY = ((double)anchor.y - (double)oldM.renderY) / (double)oldM.renderH;
        if (sourceFracX < 0.0) sourceFracX = 0.0;
        if (sourceFracX > 1.0) sourceFracX = 1.0;
        if (sourceFracY < 0.0) sourceFracY = 0.0;
        if (sourceFracY > 1.0) sourceFracY = 1.0;
    }

    g_videoZoomMultiplier = newZoom;
    StopVideoPanDrag();
    ...
    if (newZoom <= 1.001f) {
        ...
        VideoViewMetrics newM{};
        if (GetVideoViewMetrics(newM, newZoom)) {
            if (newM.maxPanX > 0) {
                double desiredX = (double)anchor.x - sourceFracX * (double)newM.renderW;
                double minX = (newM.renderW >= newM.clientW) ? (double)(newM.clientW - newM.renderW) : 0.0;
                double maxX = (newM.renderW >= newM.clientW) ? 0.0 : (double)(newM.clientW - newM.renderW);
                if (desiredX < minX) desiredX = minX;
                if (desiredX > maxX) desiredX = maxX;
                g_videoPanNormX = VideoAxisNormFromPos(desiredX, newM.renderW, newM.clientW);
            }
            else {
                g_videoPanNormX = 0.5f;
            }
            ...
            if (newM.maxPanY > 0) {
                double desiredY = (double)anchor.y - sourceFracY * (double)newM.renderH;
                double minY = (newM.renderH >= newM.clientH) ? (double)(newM.clientH - newM.renderH) : 0.0;
                double maxY = (newM.renderH >= newM.clientH) ? 0.0 : (double)(newM.clientH - newM.renderH);
                if (desiredY < minY) desiredY = minY;
                if (desiredY > maxY) desiredY = maxY;
                g_videoPanNormY = VideoAxisNormFromPos(desiredY, newM.renderH, newM.clientH);
            }
            else {
                g_videoPanNormY = 0.5f;
            }
...
static void PanVideoByKey(int dirX, int dirY) {
    if (g_videoZoomMultiplier <= 1.001f) return;

    StopVideoPanDrag();

    VideoViewMetrics m{};
    if (!GetVideoViewMetrics(m)) return;

    if (dirX != 0 && m.maxPanX > 0) {
        const double kMinStepPixels = 48.0;
        const double kStepFraction = 0.20;
        double stepPixels = (double)m.maxPanX * kStepFraction;
        if (stepPixels < kMinStepPixels) stepPixels = kMinStepPixels;
        if (stepPixels > (double)m.maxPanX) stepPixels = (double)m.maxPanX;

        g_videoPanNormX = Clamp01(g_videoPanNormX + (float)((stepPixels / (double)m.maxPanX) * (double)dirX));
    }

    if (dirY != 0 && m.maxPanY > 0) {
        const float kPanStepY = 0.10f;
        g_videoPanNormY = Clamp01(g_videoPanNormY + kPanStepY * (float)dirY);
    }

    ApplyVideoZoom();
}

static void CenterVideoPan() {
    StopVideoPanDrag();
    g_videoPanNormX = 0.5f;
    g_videoPanNormY = 0.5f;
    ApplyVideoZoom();
...
static void PlayIndex(size_t idx) {
    if (!g_vlc) {
        ...
        libvlc_video_set_key_input(g_mp, 0);
        libvlc_video_set_mouse_input(g_mp, 0);
        libvlc_video_set_scale(g_mp, 0.f);
        libvlc_video_set_aspect_ratio(g_mp, NULL);
...
static LRESULT CALLBACK CombineLogProc(HWND h, UINT m, WPARAM w, LPARAM l) {
    ...
    case WM_CLOSE:
        ShowWindow(h, SW_HIDE); // hide, do not destroy; preserve log
        return 0;
    }
    return DefWindowProcW(h, m, w, l);
}
...
static DWORD WINAPI FileOpThreadProc(LPVOID param) {
    ...
    else if (task->kind == FileOpKind::Iw3Submit) {
        const size_t total = task->srcFiles.size();
        if (task->dstFolder.empty()) {
            FileOpEmit(task, L"ERROR: Queue directory not set.\r\n");
            rc = 2;
        }
        else {
            for (size_t i = 0; i < total; ++i) {
                ...
                {
                    wchar_t hdr[256];
                    swprintf_s(hdr, L"IW3 submit %zu of %zu:\r\n", i + 1, total);
                    FileOpEmit(task, hdr);
                    FileOpEmit(task, L"  From: " + src + L"\r\n");
...

                {
                    ...
                    std::string jsonUtf8;
                    BuildIw3JobJsonUtf8(src, queuedNameOnly, task->iw3, jsonUtf8);
                    ...
                    if (!MoveFileExW(jsonTemp.c_str(), jsonFinal.c_str(),
                        MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED)) {
...
static void HandleIw3SubmitFromListSelection()
{
    if (g_view == ViewKind::Drives) return;
    ...
    if (g_cfg.topazUpscaleQueue.empty()) {
        MessageBoxW(g_hwndMain,
            ...
            L"IW3 2D?3D submit", MB_OK | MB_ICONERROR);
        return;
    }
    ...
    UINT_PTR, DWORD_PTR)
{
    ...
    if (m == WM_KEYDOWN) {
        ...
        // IW3 2D->3D submit: Ctrl+3
        if (ctrl && (w == '3' || w == VK_NUMPAD3)) {
            HandleIw3SubmitFromListSelection();
            return 0;
        }
...
static LRESULT CALLBACK VideoSubclass(HWND h, UINT m, WPARAM w, LPARAM l,
    ...
    if ((m == WM_KEYDOWN || m == WM_SYSKEYDOWN) && g_mp) {
        ...
        if (alt) {
            switch (w) {
            case VK_LEFT:  PanVideoByKey(-1, 0); return 0;
            case VK_RIGHT: PanVideoByKey(1, 0); return 0;
            case VK_UP:    PanVideoByKey(0, -1); return 0;
            case VK_DOWN:  PanVideoByKey(0, 1); return 0;
            case VK_HOME:  CenterVideoPan(); return 0;
            }
        }
        ...
        case 'Z':
            if (ctrl) { AdjustVideoZoom(true); return 0; }
            break;

        case 'X':
            if (ctrl) { AdjustVideoZoom(false); return 0; }
            break;
...
static LRESULT CALLBACK SeekSubclass(HWND h, UINT m, WPARAM w, LPARAM l,
    ...
    if (m == WM_KEYDOWN || m == WM_SYSKEYDOWN) {
        ...
        if (alt) {
            switch (w) {
            case VK_LEFT:  PanVideoByKey(-1, 0); if (g_hwndVideo) SetFocus(g_hwndVideo); return 0;
            case VK_RIGHT: PanVideoByKey(1, 0); if (g_hwndVideo) SetFocus(g_hwndVideo); return 0;
            case VK_UP:    PanVideoByKey(0, -1); if (g_hwndVideo) SetFocus(g_hwndVideo); return 0;
            case VK_DOWN:  PanVideoByKey(0, 1); if (g_hwndVideo) SetFocus(g_hwndVideo); return 0;
            case VK_HOME:  CenterVideoPan(); if (g_hwndVideo) SetFocus(g_hwndVideo); return 0;
            }
        }
        ...
        if (ctrl && (w == 'X' || w == 'x')) { AdjustVideoZoom(false); if (g_hwndVideo) SetFocus(g_hwndVideo); return 0; }
...
static void UpdateMainLayout() {
    ...
    if (g_inPlayback) {
        ...
        if (g_hwndList) ShowWindow(g_hwndList, SW_HIDE);
        if (g_hwndVideo) ShowWindow(g_hwndVideo, SW_SHOW);
        if (g_hwndVideoSurface) ShowWindow(g_hwndVideoSurface, SW_SHOW);

        if (g_fullscreen) {
            if (g_hwndSeek) ShowWindow(g_hwndSeek, SW_HIDE);
            if (g_hwndVideo) MoveWindow(g_hwndVideo, 0, 0, cx, cy, TRUE);
        }
        else {
            const int seekH = 32;
            int videoH = cy - seekH;
            ...
            int seekTop = videoH;
            int seekActualH = cy - seekTop;
            if (seekActualH < 0) seekActualH = 0;

            if (g_hwndSeek) {
                ShowWindow(g_hwndSeek, SW_SHOW);
                MoveWindow(g_hwndSeek, 0, seekTop, cx, seekActualH, TRUE);
            }
            if (g_hwndVideo) MoveWindow(g_hwndVideo, 0, 0, cx, videoH, TRUE);
        }
        ApplyVideoZoom();
        return;
    }
    ...
    if (g_hwndStatus) {
        ...
        if (statusH < 0) statusH = 0;
    }

    if (g_hwndVideo) ShowWindow(g_hwndVideo, SW_HIDE);
    if (g_hwndVideoSurface) ShowWindow(g_hwndVideoSurface, SW_HIDE);
    if (g_hwndSeek) ShowWindow(g_hwndSeek, SW_HIDE);
    if (g_hwndList) {
        ShowWindow(g_hwndList, SW_SHOW);
        MoveWindow(g_hwndList, 0, 0, cx, (cy >= statusH ? cy - statusH : 0), TRUE);
    }
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT m, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE: {
        ...
        g_hwndVideoSurface = CreateWindowExW(0, L"STATIC", L"",
            WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
            0, 0, 100, 100, g_hwndVideo, NULL, g_hInst, NULL);
    ...
    case WM_SYSKEYDOWN:
        if (g_inPlayback && (w == VK_LEFT || w == VK_RIGHT || w == VK_UP || w == VK_DOWN || w == VK_HOME)) {
            SendMessageW(g_hwndVideo, WM_SYSKEYDOWN, w, l);
            return 0;
        }
        break;
